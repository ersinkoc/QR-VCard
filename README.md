...
- **Backend** — Directus, configured only through the server's `.env`. The browser never
  talks to Directus.
- **QR codes** — generated by the Art QR API through the server (the API key never
  reaches the browser).
...
`GET /api/qr/<code>` and names nothing but the short code; the server builds
`PUBLIC_URL/c/<code>` (or the request's own origin when `PUBLIC_URL` is empty) and the
complete provider body itself, so the metered provider cannot be used to encode arbitrary
text. PNGs are cached in memory (keyed on the full link) and by browsers for a day; only
cache misses reach the provider and count against the per-client limit.
